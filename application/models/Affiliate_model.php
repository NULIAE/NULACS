...
{	
	...
	public function census_report($report_year = NULL, $affiliate_id = NULL, $status = NULL)
	{
		$this->db->select('rep.*,af.organization,af.city,af.state,cs.status,exp.field_total_expenditures,exp.field_a_salaries_wages,exp.field_b_fringe_benefits,exp.field_c_professional_fees,exp.field_d_travel,exp.field_e_postage_freight,
						   ...
						   vm.field_ypc_members,vm.field_aux_members,vm.field_guild_members,GROUP_CONCAT(p.title) as program_titles,GROUP_CONCAT(p.field_program_area_tid) as program_areas,sams.field_service_area_city_county as service_area');
		$this->db->from('census_report rep');
		$this->db->join('affiliate af', 'af.field_affiliate_select_value = rep.field_affiliate_select', 'left');
		...
		$this->db->join('service_areas sams', 'sam.pk_id = sams.fk_service_area_id','left');
