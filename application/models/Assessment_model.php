...
{	
  ...
  /**
	...
	*/
	...
	{
		$this->db->select('*');
		$this->db->from('self_assessment s');
		if(isset($data['saffiliate_id']) && !empty($data['affiliate_id'])){
			$this->db->where('s.affiliate_id', $data['affiliate_id']);
		}
		...
		$this->db->order_by('s.self_assessment_id','desc');
		$this->db->limit(5); 
		$query = $this->db->get(); 
