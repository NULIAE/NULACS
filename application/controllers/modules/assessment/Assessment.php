...
// exit;	
  ...
  public function censusexport(){
	...
	include 'application/views/export.php';
	...
	// 	// //   if (file_exists($report_data[$key]['field_photo_title'])){			
	// 	// // 	$templateProcessor->setImageValue('IMAGE', array('path' => $report_data[$key]['field_photo_title']));
	// 	// //   }else{
	// 	// // 	$templateProcessor->setValue('IMAGE', "");
	// 	// //   }
  ...
  public function censusPdfExport() {
	$data = $this->input->get();
	$dompdf = new Dompdf();
	$report_year = $data['year'] ? $data['year'] : NULL;
	$status = $data['status'] ? $data['status'] : NULL;
	$affiliate_id = $data['affiliate'] ? $data['affiliate'] : NULL;
	$report_details = $this->Affiliate_model->report_details($report_id);
	...
	if (!empty($report_data)) {
		foreach ($report_data as $report) {
			...
			$service_area_main = $this->Affiliate_model->service_areas($report['report_id']);
			$service_data = $this->Affiliate_model->service_areas_details($service_area_main[0]['pk_id']);

			$education_data = $this->Affiliate_model->education_prg($report['report_id']);
			...
			$pgm_title = explode(',',$report['program_titles']);
			$pgm_area_id = explode(',',$report['program_areas']);
			$newArray = array();
			for ($i = 0; $i < count($pgm_title); $i++) {
				$newArray[] = $pgm_title[$i] . ' = ' . $pgm_area_id[$i];
			}

			if($report['field_photo']) {
				$img = $report['field_photo'];
			} else if($report['field_photo_title']) {
				$img = $report['field_photo_title'];
			} else {
				$img = 'person-icon.png';
			}
			$path = base_url().'resources/images/profile/'.$img;
			// if($report['field_photo']){
			// 	$path = base_url().'resources/images/profile/'.$report['field_photo'];
			// }else{
			// 	$path = base_url().'resources/images/person-icon.png';
			// }
								...
								<table>
									<tbody>
										<tr>
											...
											<td>
												:&nbsp;<span>' . htmlspecialchars($report['field_date_established']) . '</span>
											</td>
										<tr>
										<tr>
											<td style="width:50%;">
			...
			<div style="margin-bottom: 1px; margin-top: 10px;font-family: Arial, sans-serif; font-size: 8pt;"><b>10.	&nbsp;&nbsp;&nbsp;&nbsp;Community Relations Activities: </b></div>
				<ul style="padding-left: 0px; margin-left: 35px;line-height:14px;font-family: Arial, sans-serif; font-size: 8pt;">
					...
					' . ($report['field_has_ad_marketing_campaign'] == 1 ? '<li>Advertising/Marketing Campaign </li>' : '') . '
					' . (isset($report['report_id']) && !empty($report['report_id']) 
						? $this->db->query("SELECT * FROM community_relation_method_ad_market WHERE community_relation_id = " . intval($report['report_id']))
							->num_rows() > 0 
								? '<li>Methods of Advertising:</li>'
								: '' 
						: '') . '
					' . ($report['field_marketing_kit_or_pamphlet'] == 1 ? '<li>Marketing Kit and/or Pamphlet</li>' : '') . '
				</ul> 
			</div>
  ...
  public function contactinfoexports()
  {
	...
	// print_r($service_data);print_r("sdfghjkljhgfdghj");
	// print_r($education_pgm_data);die;
	  ...
	  $templateProcessor->cloneBlock('SER_DATA', count($service_data));
	...
	$templateProcessor->setValue('economic_prg_ser', $economic_prg_ser);
	  ...
	  if(empty($other_pgm_data)){
		$na = "N/A";
		$templateProcessor->setValue('other_pgms', $na);

	  }else{
		$templateProcessor->setValue('other_pgms', "");
	  }
	...
	//   }
	  foreach($service_data as $service_prg_data){
		$templateProcessor->setValue('country', str_replace("&","and",$service_prg_data["field_service_area_city_county"]), 1);
		$templateProcessor->setValue('population', str_replace("&","and",$service_prg_data["field_service_area_population"]), 1);
		$templateProcessor->setValue('white', str_replace("&","and",$service_prg_data["field_service_area_white"]), 1);
		$templateProcessor->setValue('african', str_replace("&","and",$service_prg_data["field_service_area_african_am"]), 1);
		$templateProcessor->setValue('hispanic', str_replace("&","and",$service_prg_data["field_service_area_hispanic"]), 1);
		$templateProcessor->setValue('asian', str_replace("&","and",$service_prg_data["field_service_area_asian_am"]), 1);
		$templateProcessor->setValue('native', str_replace("&","and",$service_prg_data["field_service_area_native_am"]), 1);
		$templateProcessor->setValue('other', str_replace("&","and",$service_prg_data["field_service_area_other"]), 1);
	  }
	...
	//   $templateProcessor->setValue('native', $field_service_area_native_am);
	  ...
	  $own_exp= $expenditure_data[0]['field_number_properties_owned'];
	  $templateProcessor->setValue('own_exp', number_format($own_exp));	  

	  $rent_exp= $expenditure_data[0]['field_number_properties_rented'];
	  $templateProcessor->setValue('rent_exp', number_format($rent_exp));

	  $pro_value= $expenditure_data[0]['field_market_value_of_property'];
	  $templateProcessor->setValue('pro_value', "$".number_format($pro_value));
	  ...
	  $tot_budget= $revenue_data[0]['field_revenue_total_budget'];
	  $templateProcessor->setValue('tot_budget', "$".number_format($tot_budget,2));
	  ...
	  if($civic_edu_data == "1"){
		$templateProcessor->setValue('civic_edu_pgm', "voter registration : Yes");	
	  }	else{ $templateProcessor->setValue('civic_edu_pgm', "voter registration : N/A"); }
	 ...
	 // var_dump($data);die;
	  ...
	  //$templateProcessor->setImageValue('image', '<img src="opt/lampp/htdocs/nul-adm-application/resources/images/demo.jpg">');
     ...
     //$image_path = str_replace('./uploads/Documents/','',$report_data[0]['field_photo_title']);
      ...
      if($report_data[0]['field_photo_title'] != "" || $report_data[0]['field_photo_title']){
		if (file_exists('/var/www/html/nulacs/resources/images/profile/'.$field_photo_title)) {
	...
	//   }else{
    //     $templateProcessor->setImageValue('IMAGE', array('path' =>'https://148.113.6.231/resources/images/profile/'.$field_photo_title));
	//   }
