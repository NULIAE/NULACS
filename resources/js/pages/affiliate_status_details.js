...
$(function () {
	...
	$("#form-search-key-indicatorsAdd").submit(function (e) {
		e.preventDefault();
		$("#form-search-key-indicators").show();
	$("#form-key-indicators").show();
		var form = $(this);
		$("#form-key-indicators").find("input[type=text]").val("");
		var inputData = {
			affiliate_id: form.data('affiliate'),
			quarter: $("#key-quarterAdd").val(),
			year: $("#key-yearAdd").val()
		}
console.log(inputData);
		...
		$("#label-duration").html(quarterName);
		$("#key-quarter").val(inputData.quarter);
		$('#key-year').val(inputData.year);
		$.ajax({
			type: 'POST',
			url: base_url + 'module/affiliate/key-indicators/search',
			data: inputData,
	...
	//End of search
	//Key indicators search form submit
	$("#form-search-key-indicators-monthly").submit(function (e) {
		e.preventDefault();
		var form = $(this);
		$("#form-key-indicators-monthly").find("input[type=text]").val("");
		var inputData = {
			affiliate_id: form.data('affiliate'),
		quarter: $("#key-monthly").val(),
		year: $("#key-year-monthly").val()
		}
		...
		quarterName = 'Mar';
		else if(inputData.quarter == 4)
		quarterName = 'April';
		else if(inputData.quarter == 5)
		quarterName = 'May';
		else if(inputData.quarter == 6)
		quarterName = 'June';
		else if(inputData.quarter == 7)
		quarterName = 'July';
		else if(inputData.quarter == 8)
		quarterName = 'Aug';
		else if(inputData.quarter == 9)
		quarterName = 'Sep';
		else if(inputData.quarter == 10)
		quarterName = 'Oct';
		else if(inputData.quarter == 11)
		quarterName = 'Nov';
		else if(inputData.quarter == 12)
		quarterName = 'Dec';

		$("#label-duration-monthly").html(quarterName);

		$.ajax({
			type: 'POST',
			url: base_url + 'module/affiliate/key-indicators/searchmonthly',
			data: inputData,
			dataType: 'json'
		}).done(function (data) {
			if (data != null) {
				$.each(data.key_indicators_monthly, function (key, value) {
				console.log(key)
					if(data.status == '1'){
						$(".key_indicators_save_btn_u_s1").text("Approved");
						$("#key_indicators_save_btn_u1").text("Approved");
						$('.key_indicators_approve_btn1').val('Approved');
						$(".key_indicators_approve_btn1").addClass("btn btn-success");
						// $('.key_indicators_approve_btn1').prop('disabled', true);
						$('#key_indicators_save_btn1').prop('disabled', true);
						$('#key_indicators_save_btn_u1').prop('disabled', true);
						$("#key_indicators_save_btn_u1").addClass("btn btn-success");
						
						document.getElementById("fieldset_disable").disabled = true;
					}else{
						$('#key_indicators_save_btn_u1').removeClass("btn btn-success");
						$('#key_indicators_save_btn_u1').removeAttr('disabled');
						$("#key_indicators_save_btn_u1").text("SAVE");
						$("#key_indicators_save_btn_u1").addClass("btn btn-primary");
						$(".key_indicators_save_btn_u1_s1").text("SAVE");
						$('.key_indicators_approve_btn1').val('Approve');
						$(".key_indicators_approve_btn1").removeClass("btn btn-success");
						$(".key_indicators_approve_btn1").addClass("btn btn-primary");
						$('.key_indicators_approve_btn1').removeAttr('disabled');
						$('#key_indicators_save_btn1').removeAttr('disabled');
						document.getElementById("fieldset_disable").disabled = false;
					}

					if(key=='GovernmentGrants' && value){
						$("#form-key-indicators-approve1").addClass("d-block");
					}

					if(key == 'GovernmentGrants' | key == 'change_in_cash_ytd' | key == 'change_in_net_assets_in_quarter')
						$("input[name='" + key + "']").val(dollarUSLocale.format(value));
					else
						$("input[name='" + key + "']").val(value);
				...
				if (data.key_indicators_monthly.is_low_risk === 0) {
					$("#pu-dx1").attr("checked", "checked");
					$("#pu-dx1").parent("label").addClass("checked");
				} else {
					$("#pu-dx1").removeAttr("checked");
					$("#pu-dx1").parent("label").removeClass("checked");
				}

				//Radio button for F/A Findings
				if (data.key_indicators_monthly.FAFindings === 0) {
					$("#pu-dx2").attr("checked", "checked");
					$("#pu-dx2").parent("label").addClass("checked");
				} else {
					$("#pu-dx2").removeAttr("checked");
					$("#pu-dx").parent("label").removeClass("checked");
				}

				//Radio button for F/S Findings
				if (data.key_indicators_monthly.FSFindings === 0) {
					$("#pu-dx3").attr("checked", "checked");
					$("#pu-dx3").parent("label").addClass("checked");
				} else {
					$("#pu-dx3").removeAttr("checked");
					$("#pu-dx").parent("label").removeClass("checked");
				}

				//Radio button for Fed Awards
				if (data.key_indicators_monthly.FedAwards === 0) {
					$("#pu-dx4").attr("checked", "checked");
					$("#pu-dx4").parent("label").addClass("checked");
				} else {
					$("#pu-dx4").removeAttr("checked");
					$("#pu-dx").parent("label").removeClass("checked");
				}
				...
				document.getElementById("fieldset_disable").disabled = false;
				$("#form-key-indicators-approve1").removeClass("d-block");
				$("#form-key-indicators-approve1").addClass(" d-none");
				$("#pu-dx1").removeAttr("checked");
				$("#pu-dx1").parent("label").removeClass("checked");

				//Radio button remove checked for F/A Findings
				$("#pu-dx2").removeAttr("checked");
				$("#pu-dx2").parent("label").removeClass("checked");

				//Radio button remove checked for F/S Findings
				$("#pu-dx3").removeAttr("checked");
				$("#pu-dx3").parent("label").removeClass("checked");

				//Radio button remove checked for Fed Awards
				$("#pu-dx4").removeAttr("checked");
				$("#pu-dx4").parent("label").removeClass("checked");

			}
		});
	});
	$("#form-search-key-indicators-monthlyAdd").submit(function (e) {
		e.preventDefault();
		$("#form-search-key-indicators-monthly").show();
	$("#form-key-indicators-monthly").show();
		var form = $(this);
		$("#form-key-indicators-monthly").find("input[type=text]").val("");
		var inputData = {
			affiliate_id: form.data('affiliate'),
		quarter: $("#key-monthlyAdd").val(),
		year: $("#key-year-monthlyAdd").val()
		}
		...
		$("#label-duration-monthly").html(quarterName);
		$("#key-monthly").val(inputData.quarter);
		$('#key-year-monthly').val(inputData.year);
		$.ajax({
			type: 'POST',
			url: base_url + 'module/affiliate/key-indicators/searchmonthly',
			data: inputData,
	...
	$("#form-key-indicators").submit(function (e) {
		...
		$.ajax({
			...
			if(data.success){		
				showDialogBox('success', data.message);
				window.location = window.location.href + "?tab=3";
			} else {
				showDialogBox('error', data.message);
			}
		});
	...
	//Key indicators monthly form submit
	$("#form-key-indicators-monthly").submit(function (e) {
		e.preventDefault();
		var form = $(this);
		var formValues = form.serializeArray();
		var values = {}
		...
		monthName = 'Jan';
		else if(inputData.month == 2)
		monthName = 'Feb';
		else if(inputData.month == 3)
		monthName = 'Mar';
		else if(inputData.month == 4)
		monthName = 'April';
		else if(inputData.month == 5)
		monthName = 'May';
		else if(inputData.month == 6)
		monthName = 'June';
		else if(inputData.month == 7)
		monthName = 'July';
		else if(inputData.month == 8)
		monthName = 'Aug';
		else if(inputData.month == 9)
		monthName = 'Sep';
		else if(inputData.month == 10)
		monthName = 'Oct';
		else if(inputData.month == 11)
		monthName = 'Nov';
		else if(inputData.month == 12)
		monthName = 'Dec';
		$.ajax({
			type: 'POST',
			url: base_url + 'module/affiliate/key-indicators/savemonthly',
			data: inputData,
			dataType: 'json'
		}).done(function (data) {
			if(data.success){		
				...
				//window.location = window.location.href + "?tab=3";
				// console.log(inputData);
				var count = $("#self-assessment-list1").children('.card').length;
				var countNo = count+1;
				var html = '<div class="card">';
				html += '<div class="card-header">';
				html += '<div class="mb-0">';
				html += '<div class="btn wrdiv">';
				html += '<div class="row  align-items-center">';
				html += '<div class="col-3"><span class="sub text-center">'+countNo+'</span></div>';
                                            
				html += '<div class="col-5"><span class="sub text-center">'+monthName+'</span></div>';
				html += '<div class="col-4"><span class="sub text-center">'+inputData.year+'</span></div>';
				html += '<div class="col-4"><span class="sub text-center">Not Approved</span></div>';
				html += '<div class="col-4"><span class="sub text-center">';
				html += '<a class="link m-x-10 text-dark" data-rel="tooltip" id="edit_monthly" data-placement="bottom" title="" data-id="'+inputData.affiliate_id+'" data-year="'+inputData.year+'" data-quarter="'+inputData.month+'" data-original-title="Edit"><i class="i i i-create"></i></a>';
				html += '</span></div>';
				...
				html += ' </div>';
				html += '</div>';
				$('#self-assessment-list1').append(html);
				//location.reload();
				$("#form-search-key-indicators-monthly").hide();
				$("#form-key-indicators-monthly").hide();
			} else {
				showDialogBox('error', data.message);
			}
		});
	...
	//End of form submit
		...
		$("#form-key-indicators-approve1").click( function() {
			var statusval='';
			var statusvals = $(this).val();
			...
			$.ajax({
				type: 'POST',
				url: base_url + 'module/affiliate/key-indicators/approvemonthly',
				data: inputData,
				dataType: 'json'
			}).done(function (data) {
				if(statusval == '1'){
					...
					$(".key_indicators_approve_btn1").addClass("btn btn-primary");
					$('#key_indicators_save_btn1').removeAttr('disabled');
					$('#key_indicators_save_btn_s1').removeAttr('disabled');
					document.getElementById("fieldset_disable").disabled = false;
				}		
...
$('.current_ratio_blur').on('blur', function() {
	if($('#current_liabilities_v').val() == 0 && $('#current_liabilities_v').val() != ""){
		console.log($('#current_liabilities_v').val());
...
$('.current_debt_ratio_blur').on('blur', function() {
	...
	if($('#total_assets_v').val() == 0 && $('#total_assets_v').val() != ""){
		console.log($('#total_assets_v').val());
		showDialogBox('error','Division by zero not possible');
	}
...
$('.change_in_cash_ytd_blur').on('blur', function() {
	...
	var from_operations_v = 	+$('#Operations_v').val().replace(/,/g, "");
	var from_financing_v = 	+$('#Financing_v').val().replace(/,/g, "");
	var from_investing_v = +$('#Investing_v').val().replace(/,/g, "");
	...
	if(from_operations_v || from_financing_v || from_investing_v){
		...
		if($.isNumeric(f_change_in_cash_ytd_s)) { 
			$("#TotalChangeIncashflow").val(dollarUSLocale.format(f_change_in_cash_ytd_s));
		}else{
			$("#TotalChangeIncashflow").val('');
